...
TL;DR: SMES是一个可扩展的稀疏MoE框架，通过渐进式专家路由解决多任务推荐中专家激活爆炸和负载倾斜问题，已在快手大规模短视频服务中部署，显著提升了推荐效果。


<details>
  <summary>Details</summary>
Motivation: 工业推荐系统通常使用多任务学习来估计多样化的用户反馈信号，但传统的均匀参数扩展与异构任务容量需求不匹配，导致在线推理成本过高且稀疏任务收益递减。

Method: 提出SMES框架，将专家激活分解为跨任务联合选择的任务共享专家子集和任务自适应私有专家，明确限制每个实例的专家执行，同时引入全局多门负载均衡正则化器来稳定训练。

Result: 在快手大规模短视频服务中部署，支持超过4亿日活用户，在线实验显示GAUC提升0.29%，用户观看时间提升0.31%。

Conclusion: SMES通过参数稀疏化作为可扩展的多任务推荐原则性扩展范式，有效解决了稀疏MoE在多任务推荐中的关键挑战，实现了稳定且显著的性能提升。
...
TL;DR: HeMix是一个可扩展的推荐排序模型，通过自适应序列标记化和异构交互结构统一建模上下文相关和上下文无关的用户兴趣，在工业规模数据集上表现优异并已成功部署。


<details>
  <summary>Details</summary>
Motivation: 工业推荐系统中面临稀疏多字段输入和超长用户行为序列的挑战，现有方法难以同时建模上下文相关和上下文无关的用户意图，且交互机制效率低下、同质化，导致预测性能不佳。

Method: 提出HeMix模型，包含Query-Mixed兴趣提取模块（通过动态和固定查询联合建模全局和实时行为序列的上下文相关/无关兴趣）和HeteroMixer块（替代自注意力，实现高效多粒度跨特征交互，采用多头token融合、异构交互和组对齐重建流程）。

Result: 在工业规模数据集上，HeMix展现出良好的扩展性，通过参数扩展能稳定提升推荐准确性，显著优于强基线。在AMAP平台部署后获得显著在线收益：GMV提升0.61%，PV_CTR提升2.32%，UV_CVR提升0.81%。

Conclusion: HeMix通过统一自适应序列标记化和异构交互结构，有效解决了工业推荐系统中长期和实时行为序列建模的挑战，实现了可扩展的高性能推荐排序，并在实际部署中验证了其商业价值。
...
TL;DR: SARM是一个端到端的直播推荐排序架构，通过可学习的文本语义锚点直接集成到排序优化中，实现细粒度的多模态内容感知推荐，已服务4亿日活用户。


<details>
  <summary>Details</summary>
Motivation: 大规模直播推荐需要在严格的实时服务约束下精确建模非平稳的内容语义。现有两种工业方法存在根本性限制：离散语义抽象通过聚类牺牲了描述精度，而密集多模态嵌入独立提取且与排序优化弱对齐，限制了细粒度内容感知排序能力。

Method: 提出SARM端到端排序架构，将自然语言语义锚点直接集成到排序优化中。每个语义锚点表示为可学习的文本标记，与排序特征联合优化，使模型能够根据排序目标调整内容描述。采用轻量级双标记门控设计捕获领域特定的直播语义，以及非对称部署策略保持低延迟的在线训练和服务。

Result: 广泛的离线评估和大规模A/B测试显示，相比生产基线模型有持续改进。SARM已完全部署，每天服务超过4亿用户。

Conclusion: SARM通过将语义锚点直接集成到排序优化中，解决了工业直播推荐中内容语义建模的局限性，实现了细粒度的多模态内容感知推荐，并在大规模部署中证明了其有效性。
...
TL;DR: PerPEFT：一种用于多模态推荐的个性化参数高效微调策略，通过按兴趣分组用户并为每组分配独立的PEFT模块，使项目嵌入能够根据用户兴趣进行条件化


<details>
  <summary>Details</summary>
Motivation: 现有基于多模态基础模型的推荐系统存在"用户盲区"问题：项目嵌入没有考虑用户兴趣，而不同兴趣的用户关注不同的项目方面。即使使用参数高效微调(PEFT)，这个问题仍然存在。

Method: 提出PerPEFT个性化PEFT策略：1) 按兴趣将用户分组；2) 为每个用户组分配独立的PEFT模块；3) 引入专门的训练技术加强用户组条件化。该方法与PEFT方法无关，可与任何适用于多模态基础模型的PEFT方法结合。

Result: 实验表明：1) PerPEFT比最强基线提升高达15.3%(NDCG@20)；2) 在不同PEFT变体上都能带来一致增益；3) 即使加入个性化，PEFT仍然轻量，仅增加基础模型参数量的1.3%。

Conclusion: PerPEFT通过个性化PEFT策略有效解决了多模态推荐中的用户盲区问题，在显著提升推荐性能的同时保持了参数效率，为多模态基础模型在推荐系统中的个性化应用提供了有效解决方案。
...
TL;DR: 研究发现查询多样性对密集检索的影响存在冲突，提出Q-D指标量化多样性影响，发现多样性特别有利于多跳检索，查询复杂度决定最优多样性（复杂度-多样性原则CDP），基于CDP提出零样本多查询合成方法在多跳任务中达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 先前关于合成数据生成中查询多样性对密集检索影响的研究结果存在冲突，需要系统性地量化多样性影响并理解其作用机制。

Method: 设计Q-D指标量化多样性影响，在4种基准类型（31个数据集）上进行实验，特别关注多跳检索，通过内容词数量衡量查询复杂度，建立复杂度-多样性原则（CDP）。

Result: 发现查询多样性特别有利于多跳检索，多样性效益与查询复杂度强相关（r≥0.95，p<0.05，12/14条件），提出CDP原则：查询复杂度决定最优多样性，提供可操作阈值（CW>10：使用多样性；CW<7：避免）。

Conclusion: 基于CDP原则提出零样本多查询合成方法，在多跳任务中达到最先进性能，为合成数据生成中的查询多样性选择提供理论指导和实用框架。
...
TL;DR: 论文发现神经检索器存在"盲点"问题，即无法检索到与查询相关但嵌入相似度低的实体，并提出ARGUS方法通过文档增强来改善检索效果。


<details>
  <summary>Details</summary>
Motivation: 可靠的检索增强生成(RAG)系统依赖于检索器找到相关信息的能力。研究发现现有神经检索器存在"盲点"问题，即会遗漏那些与查询相关但在嵌入空间中相似度低的实体，这影响了RAG系统的鲁棒性和可信度。

Method: 提出检索概率评分(RPS)来预测盲点风险，无需昂贵的检索评估。开发ARGUS管道，通过从知识库(如维基百科首段)进行有针对性的文档增强，提高高风险(低RPS)实体的可检索性。

Result: 在BRIGHT、IMPLIRET和RAR-B数据集上的实验表明，ARGUS在所有评估的检索器上都取得了显著改进(平均+3.4 nDCG@5和+4.5 nDCG@10)，在具有挑战性的子集上提升更大。

Conclusion: 预先补救检索器的盲点对于构建鲁棒可信的RAG系统至关重要。ARGUS方法能有效识别并改善高风险实体的检索性能，为更可靠的RAG系统提供了解决方案。
...
TL;DR: DiffuReason：一种用于序列推荐的"Think-then-Diffuse"框架，通过多步思考令牌进行潜在推理，扩散过程去噪，以及端到端策略优化来提升推荐性能。


<details>
  <summary>Details</summary>
Motivation: 现有序列推荐方法使用确定性潜在链会累积噪声且忽略用户意图的不确定性，且通常采用分阶段训练阻碍联合优化和探索。

Method: 提出DiffuReason框架：1) Think阶段生成思考令牌进行潜在推理；2) Diffuse阶段通过扩散过程对中间表示进行概率性去噪；3) 使用Group Relative Policy Optimization进行端到端对齐优化。

Result: 在四个基准测试中持续改进多种骨干架构，并在大规模工业平台的在线A/B测试中验证了实际有效性。

Conclusion: DiffuReason通过统一的"思考-扩散"框架解决了序列推荐中潜在推理的噪声累积和不确定性建模问题，实现了推理与精炼模块的协同进化。
...
TL;DR: STAR：通过轨迹驱动内部化框架，将多智能体教师系统的复杂推理能力蒸馏到单一高效推荐模型中，实现实时推理增强推荐


<details>
  <summary>Details</summary>
Motivation: 大型语言模型为推荐系统带来了世界知识和语义推理能力，但如何有效整合协同信号同时避免过高推理延迟是关键瓶颈

Method: 提出轨迹驱动内部化框架：1) 设计多智能体教师系统，具备多轮工具使用和反思能力，通过协同信号翻译机制将行为模式转化为自然语言证据；2) 轨迹驱动蒸馏管道将智能体逻辑（规划、工具使用、自我反思）转移到紧凑的STAR模型中

Result: STAR在实验中超越其教师系统8.7%至39.5%，同时消除了迭代延迟，为实时推理增强推荐铺平道路

Conclusion: STAR框架成功将复杂推理能力内部化到单一高效模型中，解决了LLM推荐系统中推理延迟与协同信号整合的关键问题
...
TL;DR: QP-OneModel：一个统一的生成式LLM，用于社交网络搜索中的多任务查询理解，通过三阶段对齐策略和意图描述生成，显著提升查询处理性能。


<details>
  <summary>Details</summary>
Motivation: 传统查询处理系统使用孤立的判别模型，存在语义理解有限、维护成本高的问题。现有LLM方法通常孤立优化子任务，缺乏语义协同，且难以适应社交网络场景的非正式语言模式和业务定义。

Method: 将异构子任务重新表述为统一的序列生成范式，采用渐进式三阶段对齐策略，最终进行多奖励强化学习。创新性地生成意图描述作为高保真语义信号，增强下游任务如查询重写和排序。

Result: 离线评估显示：相比判别基线整体提升7.35%，NER任务F1提升9.01%，词权重任务提升9.31%。在未见任务上比32B模型准确率高7.60%。在线A/B测试：检索相关性(DCG)提升0.21%，用户留存率提升0.044%。

Conclusion: QP-OneModel通过统一的生成式LLM框架有效解决了社交网络查询处理的挑战，实现了语义协同和任务泛化，在小红书平台成功部署并验证了工业价值。
...
<details>
  <summary>Details</summary>
Motivation: 在生产推荐系统的初始检索阶段，存在嵌入表达力与可扩展性/延迟之间的权衡，需要既紧凑又富有表达力的表示方法。

Method: 提出训练策略学习高维稀疏嵌入层替代传统密集嵌入，修改生产级协同过滤自编码器ELSA，实现嵌入压缩。

Result: 嵌入尺寸最多减少10倍时推荐准确性无损失，最多减少100倍时仅有2.5%损失；活跃嵌入维度显示出可解释的倒排索引结构。

Conclusion: 高维稀疏嵌入层能平衡效率、表达力和可解释性，使候选检索模型本身具备分段级推荐功能。
...
TL;DR: Kunlun是一个可扩展的推荐系统架构，通过提高模型效率和资源分配，实现了可预测的幂律缩放，已在Meta Ads模型中部署并产生显著生产影响。
...
Method: 提出Kunlun架构，包括低层优化（广义点积注意力、分层种子池化、滑动窗口注意力）和高层创新（计算跳过、事件级个性化），系统性提升模型效率和资源分配。

Result: 在NVIDIA B200 GPU上将模型FLOPs利用率从17%提升到37%，缩放效率比最先进方法提高一倍，已在Meta Ads主要模型中部署并产生显著生产影响。

Conclusion: Kunlun通过系统性优化解决了推荐系统缩放效率低下的问题，实现了可预测的幂律缩放，为大规模推荐系统的资源设计和分配提供了有效解决方案。
...
TL;DR: RAGTIME是TREC的一个多语言评估赛道，旨在研究从多语言源文档生成报告，包含阿拉伯语、中文、英语和俄语新闻文档，设有三个任务类型，共有13个团队提交了125个运行结果。


<details>
  <summary>Details</summary>
Motivation: 研究从多语言源文档生成报告的问题，为多语言信息处理和报告生成提供评估框架和基准。

Method: 创建包含阿拉伯语、中文、英语和俄语新闻故事的多语言文档集合，设计三个任务类型：多语言报告生成、英语报告生成和多语言信息检索，组织13个团队参与评估。

Result: 共有13个参与团队（包括赛道协调员提供的基线系统）为三个任务提交了125个运行结果，建立了多语言报告生成的评估基准。

Conclusion: RAGTIME赛道成功建立了多语言报告生成的评估框架，为研究社区提供了有价值的基准数据集和评估结果，推动了多语言信息处理和报告生成技术的发展。
