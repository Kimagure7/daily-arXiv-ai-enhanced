...
TL;DR: SP-Rank是首个大规模公开数据集，用于评估结合一阶偏好和二阶预测的排序算法，包含12,000+数据点，涵盖地理、电影、绘画三个领域，支持九种不同子集大小的收集格式。


<details>
  <summary>Details</summary>
Motivation: 传统数据集仅捕获个体偏好（一阶信号），缺乏对他人投票预测（二阶信号）的建模能力。需要更丰富的数据集来支持在专家身份未知但假设存在的情况下，对偏好聚合进行实证分析。

Method: 创建SP-Rank数据集，每个数据点包含个人投票（一阶信号）和他人投票的元预测（二阶信号）。数据集包含12,000+人工生成数据点，涵盖三个领域和九种收集格式。提出SP-Voting方法，联合推理两个信号来推断真实排名。

Result: 结合二阶信号显著提高了准确性，优于仅使用投票的方法。在三个核心任务中表现优异：(1)完整真实排名恢复，(2)子集级排名恢复，(3)投票者行为的概率建模。

Conclusion: SP-Rank数据集填补了结合一阶和二阶信号进行排序研究的空白，支持学习排序、从嘈杂众包中提取专家知识、偏好微调中的奖励模型训练等下游应用，促进人类偏好建模、聚合理论和人机对齐研究。
...
TL;DR: TagRAG是一个标签引导的层次知识图谱RAG框架，通过构建领域标签链实现高效全局推理和可扩展的图维护，相比传统RAG和GraphRAG在性能和效率上有显著提升。


<details>
  <summary>Details</summary>
Motivation: 传统RAG方法依赖片段级检索，难以处理查询聚焦的摘要任务；GraphRAG虽然引入图推理但存在信息提取效率低、资源消耗大、增量更新适应性差等问题。

Method: 提出TagRAG框架，包含两个核心组件：1) 标签知识图谱构建 - 从文档提取对象标签及其关系，组织成层次化领域标签链；2) 标签引导的检索增强生成 - 检索领域中心的标签链来定位和合成相关知识。

Result: 在UltraDomain数据集（农业、计算机科学、法律和跨领域）上的实验显示，TagRAG相比基线方法平均胜率达到95.41%，同时构建效率比GraphRAG高约14.6倍，检索效率高1.9倍。

Conclusion: TagRAG通过标签引导的层次知识图谱方法有效解决了传统RAG和GraphRAG的局限性，实现了高效全局推理、更好的检索粒度、对小语言模型的适应性以及高效的知识增量更新。
...
TL;DR: CourtNav是一个语音引导的法律PDF导航系统，通过语音命令快速定位文档段落，将查找时间从数分钟缩短到数十秒


<details>
  <summary>Details</summary>
Motivation: 司法工作需要阅读冗长的法律文档，但法官在庭审中时间有限，无法详细阅读数百页的材料。特别是在印度，判决书和交叉询问记录尤其冗长，需要高效的工具来快速定位关键内容

Method: 开发了CourtNav系统：语音转录命令，使用语法优先（精确正则匹配）和基于LLM的路由器（使用少量示例进行分类），在布局感知的混合索引上进行检索，自动滚动到目标段落并高亮显示。界面只显示有依据的段落，不显示自由文本

Result: 在代表性起诉书、诉状和命令的试点中，中位相关时间从3-5分钟（手动导航）降至10-15秒；包含快速视觉验证后为30-45秒。在固定时间预算下，这种导航优先设计增加了实际查阅的记录广度

Conclusion: CourtNav通过语音导航显著提高了法律文档查阅效率，同时保持了证据的可验证性和可审计性，为司法工作提供了实用的工具支持
...
TL;DR: 本文提出KP-Agent，一个基于LLM的智能体系统，用于赞助搜索广告中的关键词修剪，通过上下文赌博机框架和强化学习优化关键词集，实验显示能提升累计利润达49.28%。


<details>
  <summary>Details</summary>
Motivation: 赞助搜索广告中，广告主需要不断调整关键词策略。虽然竞价调整和关键词生成已有较多研究，但关键词修剪（优化关键词集以提升广告效果）仍未被充分探索。现有实践存在效率低下的问题，这在美团平台医药广告商的50万条SSA记录数据集中得到证实。

Method: 提出KP-Agent，一个基于大语言模型的智能体系统，包含领域工具集和记忆模块。通过将关键词修剪建模为上下文赌博机问题，KP-Agent生成代码片段，通过强化学习来优化关键词集。

Result: 实验结果表明，KP-Agent相比基线方法，能够提升累计利润最高达49.28%。

Conclusion: KP-Agent系统有效解决了赞助搜索广告中关键词修剪的挑战，通过LLM智能体结合强化学习框架，显著提升了广告活动的利润表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有对话系统难以有效处理新闻相关的趋势查询，需要专门针对对话场景的趋势检测方法，而传统搜索引擎的方法在对话场景中表现不佳

Method: 多阶段框架：1) 利用热点事件生成索引查询；2) 采用检索匹配机制进行实时在线检测；3) 引入级联召回和排序架构平衡效率与准确性；4) 使用单召回模块作为冷启动策略收集数据微调重排序器

Result: 在离线评估和在线A/B测试中显著优于基线方法，用户满意度在正负反馈比方面相对提升27%

Conclusion: 该框架有效解决了对话系统中趋势查询检测的问题，通过系统优化显著提升了用户体验，为对话系统的趋势检测提供了实用解决方案
...
TL;DR: 提出基于提示工程和思维链任务分解的单模型框架，用于多语言电商搜索中的类目相关性判断，在保持高准确率的同时显著降低训练和推理复杂度。


<details>
  <summary>Details</summary>
Motivation: 传统集成系统虽然能提高多语言电商搜索中类目相关性判断的准确性，但存在训练、推理和维护复杂度高的问题，需要更轻量高效的解决方案。

Method: 使用思维链任务分解将相关性判断过程分为四个可解释子任务（翻译、意图理解、类目匹配、相关性判断），并基于Qwen2.5-14B模型采用LoRA进行轻量微调。

Result: 在CIKM 2025 AnalytiCup竞赛中，公开榜得分0.8902，私有榜得分0.8889；单A100 GPU上推理速度达20样本/秒，在保持竞争力的同时显著提升效率。

Conclusion: 结构化提示结合轻量微调的单模型框架能够超越复杂集成系统，为可扩展的工业AI应用提供了新范式，平衡了准确性、效率和可解释性。
...
<details>
  <summary>Details</summary>
Motivation: RAG系统虽然能提升LLM的准确性和时效性，但引入了事实不一致、来源冲突、偏见传播和安全漏洞等问题，削弱了系统的可信度。当前RAG评估缺乏量化单个检索文档对最终输出贡献的指标。

Method: 基于部分信息分解（Partial Information Decomposition）提出了影响力分数（IS）这一新度量标准。通过两个实验验证：1）在三个数据集上进行毒化攻击模拟；2）进行消融研究，比较仅使用IS排名靠前文档与剩余文档生成的响应与原响应的相似度。

Result: 毒化攻击实验中，IS在86%的情况下正确识别出恶意文档为最具影响力的文档。消融研究表明，仅使用IS排名靠前文档生成的响应比使用剩余文档生成的响应更接近原始响应。

Conclusion: IS能有效隔离和量化文档影响力，为提高RAG系统的透明度和可靠性提供了有价值的工具。
...
TL;DR: 提出一个结合个性化排名与摘要生成的框架，通过分析用户历史评论建立语义偏好档案，匹配新评论的情感与主题相似度，生成个性化摘要以提升决策效率。


<details>
  <summary>Details</summary>
Motivation: 在线消费者评论数量激增导致信息过载，现有排名系统依赖有用性投票、星级评分和时效性等通用指标，无法捕捉个体用户兴趣，且通常将文本情感与评分信号分开处理。

Method: 1) 通过星级评分与评论文本的混合分析建模用户情感；2) 使用句子嵌入和聚类从历史评论中提取用户偏好，形成主题和情感维度的语义档案；3) 基于情感和方面相似度的相关性评分算法匹配档案与新评论；4) 对匹配度高的评论进行摘要生成。

Result: 70名参与者的用户研究表明，个性化方法提高了满意度、感知相关性和决策信心，同时减少了阅读时间。结果证明该方法能有效缓解信息过载，提供符合用户特定偏好的内容。

Conclusion: 该研究提出的个性化框架通过整合评论排名和摘要生成，显著提升了评论丰富决策环境中的用户体验，强调了个性化内容在信息过载场景中的价值。
...
<details>
  <summary>Details</summary>
Motivation: 现代密集信息检索模型通常依赖成本高昂的大规模预训练，需要更高效的IR模型构建方法

Method: 提出LLM2IR框架，通过无监督对比学习将任何解码器专用LLM转换为信息检索模型

Result: 在不同LLM和多个IR基准测试（LoCo、LongEmbed、BEIR）上证明有效性，发现模型上下文长度与IR能力正相关
...
TL;DR: 将时间弯曲编辑距离（TWED）推广到任意度量空间，提出广义TWED（GTWED），保持度量性质，适用于符号数据、流形等任意域上的序列。


<details>
  <summary>Details</summary>
Motivation: Marteau (2009)提出的TWED主要针对实数向量时间序列，限制了其在符号数据、流形、嵌入等更广泛序列类型上的应用。需要将TWED推广到任意度量空间，使其能处理更一般的数据类型。

Method: 将观测域和时域都视为度量空间(X,d)和(T,Δ)，在此基础上定义广义TWED（GTWED）。提供完整的度量性质证明，并展示当X=ℝ^d、T⊂ℝ、g(x)=x时，GTWED退化为经典TWED。

Result: GTWED在温和假设下保持真正的度量性质（非负性、对称性、三角不等式）。理论框架使得TWED类度量能应用于任意域上的序列，如符号数据、流形或嵌入空间。

Conclusion: 成功将TWED推广到任意度量空间，建立了GTWED的理论基础。这一推广扩展了弹性距离的应用范围，使其能处理更广泛的序列数据类型，为在非传统时间序列领域应用TWED类度量提供了理论支持。
...
TL;DR: 本文对2018-2025年间的检索增强生成（RAG）架构进行了系统性文献综述，提供了现代RAG的实践指南和详细概述，包括统一分类法、评估框架及部署建议。


<details>
  <summary>Details</summary>
Motivation: 随着LLM系统扩展，RAG提供了一种模块化方法来集成外部知识而无需增加模型容量。然而，由于RAG方法的多样性（包括融合机制、检索策略和编排方法），研究和工程实践变得碎片化，需要系统性的整合和指导。

Method: 采用系统性文献综述方法，涵盖2018-2025年间的学术研究、工业应用和实际部署。将现有RAG技术整合为统一分类法，提供定量评估框架，并分析信任和对齐的影响。

Result: 创建了RAG技术的统一分类体系，开发了定量评估框架，分析了信任和对齐的实践意义，为构建弹性、安全、领域自适应的RAG系统提供了实用框架。

Conclusion: 本文为RAG系统的部署提供了全面的技术参考和实践框架，综合了学术文献、行业报告和技术实施指南的见解，有助于指导稳健、安全且可适应不同领域的RAG系统建设。
...
<details>
  <summary>Details</summary>
Motivation: 当前文档分块方法单独处理每个文档，但复杂查询需要跨多个来源的信息，存在知识碎片化问题。需要一种能在语料库层面重构知识的方法。

Method: 提出跨文档主题对齐(CDTA)分块方法：1) 识别跨文档的主题；2) 将文档片段映射到相应主题；3) 将这些片段合成为统一的知识块。

Result: 在HotpotQA多跳推理任务中，CDTA达到0.93忠实度，比上下文检索(0.83)和语义分块(0.78)显著提升12%。在UAE法律文本上达到0.94忠实度和0.93引用准确率。当k=3时仍保持0.91忠实度，而语义方法降至0.68。

Conclusion: 虽然索引成本较高，但CDTA生成信息密集的知识块，减少了查询时的检索需求。对于高查询量且知识分散的应用，跨文档合成比文档内优化有显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 工业零件规格从非结构化文本中的提取在制造、采购和维护中仍面临挑战，手动处理耗时且易出错，需要更准确高效的自动化解决方案。

Method: 采用检索增强的多LLM集成框架，协调9个最先进的LLM（包括Gemini、OpenAI、Mistral、Gemma系列），构建三阶段流水线：并行提取、针对性研究增强、智能合成与冲突解决，结合FAISS语义检索提供事实数据支持。

Result: 在真实工业数据集上的实验结果表明，相比领先的单LLM基线，在提取准确性、技术完整性和结构化输出质量方面取得显著提升。

Conclusion: RAGsemble框架为工业领域提供了可扩展的集成架构、全流程的RAG集成、全面的质量评估机制，以及适用于知识密集型制造环境的即用型解决方案。
...
TL;DR: 提出一个可解释的聚合框架，结合LLM评分与层次分析法，用于将用户定义的标准转化为可量化的评估指标


<details>
  <summary>Details</summary>
Motivation: 评估复杂文本需要将用户定义标准转化为可量化、可解释的指标，现有方法存在复杂性、延迟问题或依赖不透明的黑盒聚合方法

Method: 结合LLM评分与层次分析法：1) 通过LLM作为评判者生成特定标准评分；2) 使用Jensen-Shannon距离测量判别能力；3) 通过AHP成对比较矩阵推导统计基础权重

Result: 在亚马逊评论质量评估和抑郁相关文本评分实验中，该方法实现了高可解释性和操作效率，同时保持可比较的预测能力，适合实时延迟敏感的网络服务

Conclusion: 提出的可解释聚合框架能够有效解决复杂文本评估中的标准量化问题，平衡了可解释性、效率和预测性能，适用于实际应用场景
...
<details>
  <summary>Details</summary>
Motivation: 传统检索系统通常将语义理解和几何搜索耦合在一起，本文旨在分离这两个方面，创建一个无需训练参数的检索框架，能够在PubMed规模（约4000万条记录）上实现高效检索。
...
Result: 系统能够在整个MEDLINE语料库（约4000万条记录）上检索出连贯的生物医学聚类。评估采用纯几何方法：头部余弦相似度、紧密度、质心闭合性和各向同性，与随机向量基线进行比较。

Conclusion: 该框架成功地将语义解释与度量几何分离，实现了无需训练参数的大规模生物医学检索系统，通过几何评估验证了检索质量，为大规模文档检索提供了新的方法。
...
TL;DR: 提出一个快速可扩展的AI系统，用于从数字化手稿中检测、提取和描述插图，应用于梵蒂冈图书馆等收藏，处理超过300万页，提取20多万张插图。


<details>
  <summary>Details</summary>
Motivation: AI革命为人文学科带来变革可能，但数字化手稿中的视觉内容仍难以大规模系统研究。现有数字档案虽提供访问，但大规模插图分析仍具挑战。

Method: 三阶段流水线：1) 微调图像分类模型过滤纯文本页；2) 高效目标检测模型识别并裁剪插图；3) 多模态图像描述模型生成简洁可读描述。结果存入可搜索数据库。

Result: 处理超过300万页数字化手稿，自动识别提取超过20万张独特插图，处理速度每页不到0.06秒，在效率和可访问性上显著优于传统分割技术。

Conclusion: 前沿AI工具能深刻重塑学术工作流程，为数字手稿时代的多学科研究开辟新途径，赋能历史研究、艺术史和文化遗产领域的学者。
...
<details>
  <summary>Details</summary>
Motivation: 现代RAG系统面临架构矛盾：向量索引优化查询延迟但难以处理持续知识更新，而数据湖擅长版本管理但引入查询延迟惩罚。需要同时满足实时语义搜索、高效知识更新和合规性要求。

Method: 提出三核心架构贡献：1) 基于SHA-256哈希的内容寻址分块级同步，实现确定性变更检测；2) 双层级存储分离热层向量索引(Milvus+HNSW)和冷层级列式版本管理(Delta Lake+Parquet)；3) 时间查询路由通过delta版本化实现跨层ACID一致性的时间点知识检索。

Result: 在100文档5个时间点的评估显示：1) 更新时仅需重新处理10-15%内容(相比完全重索引的100%)；2) 当前知识检索延迟低于100ms；3) 跨版本历史的时间查询延迟低于2秒；4) 通过热/冷层分离优化存储成本。

Conclusion: LiveVectorLake架构使生产RAG部署能够同时优化查询性能、更新效率和监管合规性，解决了向量索引与数据湖架构的固有矛盾。
...
TL;DR: 提出了一个推理式对话检索基准，包含707个对话和11个领域，通过分解验证框架确保质量，结果显示结合对话历史和推理能显著提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准将多轮对话和推理密集型检索分开处理，但现实世界的信息寻求需要两者结合，因此需要建立能够同时评估对话和推理能力的基准。

Method: 采用分解验证框架，将复杂查询转化为基于事实的多轮对话，通过多层次验证确保质量：原子事实与来源验证，并为每个对话轮次生成明确的检索推理。

Result: 结合对话历史和推理能将检索性能翻倍（从0.236提升到0.479 nDCG@10），推理专用模型显著优于密集编码器，但隐式推理仍然具有挑战性。

Conclusion: 该基准填补了对话检索与推理评估的空白，展示了结合对话历史和推理的重要性，同时指出了隐式推理特别是逻辑连接未明确表述时的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型重塑了用户搜索认知，从离散关键词搜索转向高维对话交互。现有电商搜索架构难以适应这一变化：精确自然语言描述常导致零结果，而简化查询又会产生噪声过多的通用结果。

Method: LEAPS采用"扩展-精炼"范式，在搜索管道两端添加插件：1) 上游查询扩展器作为意图翻译器，采用三阶段训练策略（逆数据增强、后验知识监督微调、多样性感知强化学习）生成自适应互补查询组合；2) 下游相关性验证器作为语义把关器，通过多源数据合成和思维链推理精确过滤噪声。

Result: 离线和在线A/B测试表明LEAPS显著提升了对话式搜索体验。其非侵入式架构既保持了针对短文本查询优化的检索性能，又能低成本集成到不同后端。自2025年8月起在淘宝AI搜索全面部署，每月服务数亿用户。

Conclusion: LEAPS成功解决了电商搜索中精确自然语言查询与简化查询之间的两难困境，通过LLM赋能的自适应插件实现了传统搜索系统向对话式搜索的无缝升级，在保持现有性能的同时显著提升了用户体验。
...
TL;DR: TMRL是一种高效的时间感知检索方法，通过Matryoshka嵌入的嵌套结构引入时间子空间，增强时间编码同时保持语义表示，在时间检索和RAG任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有检索器在时间检索增强生成系统中是关键瓶颈，无法检索时间相关上下文会降低下游生成质量，无论LLM推理能力如何。

Method: 提出时间感知Matryoshka表示学习(TMRL)，利用Matryoshka嵌入的嵌套结构引入时间子空间，增强时间编码同时保持通用语义表示。

Result: TMRL能高效适配多种文本嵌入模型，在时间检索和时间RAG任务上相比先前非时间Matryoshka方法和时间方法都表现优异，支持灵活的准确率-效率权衡。

Conclusion: TMRL为时间检索增强生成系统提供了一种高效的时间感知检索解决方案，通过Matryoshka嵌入的嵌套结构实现了时间编码和语义表示的平衡。
...
TL;DR: 论文提出SToICaL损失函数，用于改进基于LLM的点式生成排序方法，通过结合项目级和标记级监督来提升排序性能。


<details>
  <summary>Details</summary>
Motivation: 虽然预训练LLM在排序任务上表现出色，但现有方法主要依赖下一个标记预测，这种损失函数本质上是与排序无关的（特别是在点式监督下）。需要一种能够融入排序感知监督的损失函数来提升LLM在信息检索中的排序性能。

Method: 提出SToICaL（Simple Token-Item Calibrated Loss）损失函数，在点式生成排序框架下，同时结合项目级和标记级的排序感知监督。首先证明了多标记文档ID的点式生成排序表达能力优于双编码器。

Result: 在WordNet和ESCI数据集上的实验表明，SToICaL的两个变体成功抑制了无效文档ID的生成概率，并在常见排序指标上超越了top-1检索性能。

Conclusion: SToICaL损失函数能够有效提升LLM在点式生成排序中的性能，通过结合不同层次的监督信息来改进排序效果，为LLM在信息检索领域的应用提供了新的优化方向。
...
TL;DR: LLMs在音频播客检索中的相关性标注与人类专家的一致性分析，发现LLMs与人类专家的一致性高于单个TREC评估者，验证了多评估者评估的重要性。


<details>
  <summary>Details</summary>
Motivation: 当前研究对LLMs在信息检索中标注相关性的能力存在争议，且主要集中在传统文本检索场景。本研究旨在探索LLMs在音频播客检索场景中与人类专家的标注一致性，以及分歧对系统排名的影响。

Method: 使用TREC 2020和2021播客轨道数据集（音频转录为2分钟片段），采用5种不同LLM模型重新评估所有查询-片段对（原由TREC评估者标注）。对LLMs与TREC评估者分歧最大的子集进行重新评估，比较人类专家与LLMs和TREC评估者的一致性。

Result: 人类专家更倾向于同意LLMs的标注而非TREC评估者。结果验证了Sormunen（2002）的见解：依赖单一评估者会导致较低的用户一致性。

Conclusion: 在音频播客检索场景中，LLMs能够提供与人类专家高度一致的相关性标注，且多评估者评估策略对于提高标注一致性至关重要。这为LLMs在复杂多媒体检索任务中的应用提供了实证支持。
...
<details>
  <summary>Details</summary>
Motivation: 高维稠密嵌入存在噪声和冗余维度，现有DIME方法需要预先通过网格搜索选择维度，成本高昂且不够灵活

Method: 基于统计准则，在推理时直接为每个查询识别最优维度集合，无需预先选择固定维度

Result: 在不同模型和数据集上保持检索效果的同时，平均减少约50%的嵌入大小

Conclusion: 提出的统计准则方法比DIME更高效灵活，能在推理时动态优化嵌入维度，显著减少存储和计算开销
