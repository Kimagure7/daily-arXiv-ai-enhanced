...
TL;DR: OxygenREC是一个工业推荐系统，采用快慢思维架构，结合LLM深度推理和高效率编码器-解码器，解决传统推荐系统多阶段优化不一致、多场景扩展性差的问题。


<details>
  <summary>Details</summary>
Motivation: 传统推荐系统存在多阶段优化目标不一致的问题，生成式推荐虽然缓解了这一问题，但仍依赖归纳模式匹配，缺乏基于世界知识的演绎推理能力。LLM虽有深度推理能力，但延迟和计算成本高，且现有方案在多场景扩展性上存在瓶颈，需要为每个场景独立训练部署，资源利用率低、维护成本高。

Method: 1. 采用快慢思维架构：慢思维使用近线LLM管道合成上下文推理指令，快思维使用高效率编码器-解码器主干进行实时生成。2. 引入语义对齐机制：通过指令引导检索过滤意图相关历史行为，使用查询到项目损失确保指令-项目一致性。3. 多场景扩展性解决方案：将场景信息转换为可控指令，使用统一奖励映射和软自适应组剪裁策略优化来对齐不同业务目标，实现"一次训练，到处部署"范式。

Result: 论文展示了OxygenREC系统能够实现深度推理同时满足严格延迟要求，解决多场景扩展性问题，提高资源利用率，降低维护成本。

Conclusion: OxygenREC通过快慢思维架构、语义对齐机制和多场景扩展性解决方案，成功解决了传统推荐系统和现有生成式推荐方法的局限性，为工业推荐系统提供了高效、可扩展的解决方案。
...
TL;DR: 开发了一个实时从新闻中填充铁路道口事故表格的演示系统，解决表格视觉不规则和新闻噪音问题


<details>
  <summary>Details</summary>
Motivation: 地方铁路委员会需要在公路-铁路平交道口事故后获得及时的情境感知，但官方FRA调查可能需要数天到数周时间

Method: 设计了一个管道：首先使用视觉语言模型将表格转换为JSON模式，然后按照表格布局意图进行分组问答以减少歧义；构建了评估数据集

Result: 评估了系统在信息检索准确性和覆盖范围方面与各种替代方案的对比

Conclusion: 提出了一个能够实时从新闻中填充铁路道口事故表格的系统，解决了表格复杂性和新闻噪音的挑战
