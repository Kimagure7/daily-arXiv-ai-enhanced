...
TL;DR: 提出基于BERT跨模态Transformer的多模态情绪意图识别模型MMEI，用于云原生AIGC推荐系统，通过融合视觉、听觉和文本模态提升情感感知推荐效果。


<details>
  <summary>Details</summary>
Motivation: 传统推荐系统主要依赖用户行为数据（点击、观看、评分），忽视了用户在内容交互过程中的实时情绪和意图状态。随着AIGC在音乐、视频、文学等领域的快速增长，对情感感知推荐系统的需求日益重要。

Method: 提出多模态情绪意图识别模型MMEI，基于BERT跨模态Transformer和注意力融合机制。系统联合处理视觉（面部表情）、听觉（语音语调）和文本（评论或话语）三种模态，使用预训练编码器ViT、Wav2Vec2和BERT，通过注意力融合模块学习情绪-意图表示，最后通过上下文匹配层驱动个性化内容推荐。
...
Conclusion: 该工作展示了跨模态情感智能在下一代AIGC生态系统中的潜力，能够实现自适应、共情和上下文感知的推荐体验，有效将AI生成内容与用户情感和意图状态对齐。
...
<details>
  <summary>Details</summary>
Motivation: 基于Transformer的生成式推荐通常需要将物品离散化为多码表示（如4个或更多码标记），这会显著增加原始物品序列的长度。这种扩展给Transformer模型带来了挑战，因为模型倾向于过度关注不相关或噪声上下文，难以有效建模带有固有噪声的用户行为序列。

Method: 提出了FAIR框架，包含三个核心组件：1）聚焦注意力机制，集成到标准Transformer中，学习两组独立的Q和K注意力权重，计算它们的差异作为最终注意力分数，以消除注意力噪声并聚焦相关上下文；2）噪声鲁棒性目标，鼓励模型在随机扰动下保持稳定的注意力模式，防止因噪声而向不相关上下文偏移；3）互信息最大化目标，指导模型识别对下一物品预测最信息丰富的上下文。

Result: 在四个公开基准上验证了FAIR的有效性，证明了其相比现有方法的优越性能。

Conclusion: FAIR是第一个采用聚焦注意力的生成式推荐框架，通过创新的注意力机制和训练目标，有效解决了多码表示带来的序列长度扩展和噪声问题，提升了用户行为建模的效果。
